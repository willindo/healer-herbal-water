...
} from "./LayoutOrchestra";

export type LayoutOverrideMap = Partial<
  Record<LayoutType, Partial<LayoutConfig>>
>;

type Controlled =
  | {
      value: LayoutType;
      onChange: (next: LayoutType) => void;
      defaultValue?: never;
    }
  | {
      value?: never;
...
export function LayoutSwitcher({
  ...
  const current: LayoutType =
    "value" in ctrl && ctrl.value ? ctrl.value : inner;
  ...
  const childCfg = keepChildConfig
    ? ((children.props as LayoutOrchestraProps).config ?? {})
    : {};
...
export function LayoutSwitcherButtons({
  ...
  return (
    <div className="flex gap-2 flex-wrap mb-3 bg-[#6cc6d0] p-3 rounded-md justify-center">
      {layouts.map((l) => (
        <button
          key={l}
          onClick={() => onSelect(l)}
