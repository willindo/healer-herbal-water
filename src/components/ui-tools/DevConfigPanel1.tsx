...
export function DevConfigPanel1<T extends Record<string, any>>({
  ...
  return (
    <div className={`fixed z-50 ${panelPos[position]}`}>
      <button
        className="bg-[#2c40a6] text-white text-sm px-2 py-1 rounded shadow"
      ...
      {open && (
        <div className="mt-2 w-64 bg-[#84257c] shadow-lg rounded-lg p-3 space-y-3 border text-[#45ec17]">
          {schema.map((field) => {
            switch (field.type) {
              case "number":
                return (
